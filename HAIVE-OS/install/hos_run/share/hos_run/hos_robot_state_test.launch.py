...
def generate_launch_description():
    ...
    wait_for_state_layer_s_arg = DeclareLaunchArgument(
        ...
        default_value=['22.0'],
        description="Time to wait for state manager node to start.",
    )
    # Node arguments
    ...
    start_rosbridge_server = IncludeLaunchDescription(
    ...
    start_device_manager = TimerAction(period=wait_for_rosbridge_server_s, actions=[
        Node(
    ...
    start_test_state_protocol = TimerAction(period=wait_for_state_layer_s, actions=[
        Node(
            package='hos_robot_state',
            executable='test_state_protocol',
            name='test_state_protocol',
            output='screen',
            emulate_tty=True,
            arguments=['-f', device_manager_fleet, device_manager_local_db]
        )
    ...
    ld.add_action(start_test_state_protocol)
